...
class Security1 implements ProvSecurity {
  ...
  Future<void> _generateKey() async {
    final algo = X25519();
    clientKey = await algo.newKeyPair();
  }

  List<int> _xor(List<int> a, List<int> b) {
    List<int> ret = List<int>.filled(max(a.length, b.length), 0);
  ...
  Future<SessionData> setup0Request() async {
    ...
    var pubKey = await clientKey!.extractPublicKey();
    sc0.clientPubkey = pubKey.bytes;
    Sec1Payload sec1 = Sec1Payload();
    sec1.sc0 = sc0;
    setupRequest.sec1 = sec1;
    _verbose(
        'setup0Request: clientPubkey = ${pubKey.bytes.toString()}');
    return setupRequest;
  }
  ...
  Future<SessionData> setup0Response(SessionData responseData) async {
    ...
    final shaAlgo = Sha256();
    var sink = shaAlgo.newHashSink();
    ...
    _verbose(
      ...
      await crypt.init(Uint8List.fromList(sharedK), Uint8List.fromList(deviceRandom!));
    _verbose(
