...
class EspProv {
  ...
  Future<void> establishSession() async {
    ...
    while (true) {
      var request = await security.securitySession(responseData ?? SessionData());
      if (request == null) {
        break;
      }
      var response =
  ...
  Future<List<WifiAP>> startScanWiFi({bool blocking = true}) async {
    return await scan(blocking: blocking);
  }
  ...
  Future<List<WifiAP>> scan(
      {bool blocking = false, // Changed default to false to prevent BLE timeout on Android
      bool passive = false,
      int groupChannels = 5,
      int periodMs = 0}) async {
    // Inicia o scan no ESP32
    ...
    // Se não for bloqueante (Android), podemos fazer polling. 
    // Se for bloqueante (Linux), o pacote startScanRequest já travou o código
    // até o scan terminar, então o status retornado já deve ser final.
    if (!blocking) {
      int retries = 0;
      while (!status.respScanStatus.scanFinished && retries < 15) {
        await Future.delayed(const Duration(milliseconds: 500));
        status = await scanStatusRequest();
        retries++;
      }
    } else {
      // Pequena trava de segurança caso o blocking retorne rápido demais
      int retries = 0;
      while (!status.respScanStatus.scanFinished && retries < 5) {
